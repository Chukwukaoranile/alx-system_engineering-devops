...
* In order to spin up a Gunicorn instance as a detached process you can use the terminal multiplexer utility tmux. Enter the command <p style="color:red;">tmux new-session -d 'gunicorn --bind 0.0.0.0:5001 web_flask.6-number_odd_or_even:app'</p> and if successful you should see no output to the screen. You can verify that the process has been created by running **pgrep gunicorn** to see its PID. Once you’re ready to end the process you can either run **tmux a** to reattach to the processes, or you can run **kill <PID>** to terminate the background process by ID.
